...
const BLOCK_TERMS = [
  ...
  "borsa","dolar","enflasyon","hava durumu","siyaset","politika",
  // Suç / şiddet (spor haberlerinde geçse de engelle)
  "bıçaklandı","bıçak","cinayet","saldırı","bomba","tutuklama","gözaltı",
  "stabbed","stabbing","shooting","arrested","murder","attack",
  // Hastalık / kişisel (efsane futbolcuların sağlığı dahil)
  "kanser","cancer","hastalık","ameliyat","öldü","vefat","hayatını kaybetti",
  "korona","covid",
  // Diğer sporlar
  "rugby","cricket","nba","basketbol","tenis","tennis",
  "formula","f1","voleybol","boks","ufc","mma","golf","yüzme","atletizm",
  // Kadın futbolu (erkek futbol sitesineyiz)
...
// Güçlü futbol terimleri — en az BİRİ geçmeli
const STRONG_FOOTBALL = [
  "süper lig","premier lig","bundesliga","serie a","la liga","ligue 1",
  "şampiyonlar ligi","europa league","uefa","fifa","dünya kupası","world cup",
  "fenerbahçe","galatasaray","beşiktaş","trabzonspor","çorum",
  "real madrid","barcelona","arsenal","chelsea","liverpool","manchester","psg","bayern","juventus",
  "transfer","imza","teknik direktör","coach","manager","forvet","kaleci","futbolcu","oyuncu",
  "milli takım","national team","maç","goal","gol","skor","penaltı","fikstür",
];

function isFootball(text) {
  const t = text.toLowerCase();
  // Kesin engel — bunlar varsa her zaman çıkar
  if (BLOCK_TERMS.some(b => t.includes(b))) return false;
  // En az bir güçlü futbol terimi geçmeli
  return STRONG_FOOTBALL.some(f => t.includes(f));
}
...
// ─── Maç eşleme ─────────────────────────────────────────────────────────────
// Stale "IN_PLAY" tespiti: 3.5 saatten eski "canlı" maç aslında bitti
function resolveTimeframe(status, utcDate, explicitTimeframe) {
  if (explicitTimeframe) {
    // Verilen timeframe'i doğrula
    if ((status === "IN_PLAY" || status === "PAUSED") && explicitTimeframe === "live") {
      const elapsed = Date.now() - new Date(utcDate).getTime();
      if (elapsed > 3.5 * 3600 * 1000) return "past";
    }
...
function mapMatch(m, leagueCode, timeframe) {
  ...
  const tf  = resolveTimeframe(m.status, m.utcDate, timeframe);
  const finished = m.status === "FINISHED" || tf === "past";
  const live = tf === "live";
  return {
    id: m.id,
    ...
    // Form + analiz alanları — attachAnalysis tarafından doldurulur
...
async function fetchLeagueMatches(leagueCode) {
  ...
  try {
    ...
    // timeframe, resolveTimeframe tarafından hesaplanır (stale-live fix dahil)
    return matches.map(m => mapMatch(m, leagueCode, null));
  } catch (err) {
...
// H2H — /v4/matches/{id}/head2head (ücretsiz endpoint)
async function fetchH2H(matchId) {
  if (!FOOTBALL_API_KEY || !matchId) return null;
  try {
    const res = await fetch(`${FOOTBALL_API_BASE}/matches/${matchId}/head2head?limit=10`, AUTH);
    if (!res.ok) return null;
    const data = await res.json();
...
async function attachAnalysis(match, existingById) {
  ...
  // Önceki çalışmadan form ve H2H de varsa yeniden çekme
  if (prev && prev.analysis && prev.homeForm) {
    match.analysis     = prev.analysis;
    match.homeForm     = prev.homeForm;
    match.awayForm     = prev.awayForm;
    match.homeGoalAvg  = prev.homeGoalAvg;
    match.awayGoalAvg  = prev.awayGoalAvg;
    match.headToHead   = prev.headToHead;
    return match;
  }
  ...
  // Form verilerini doğrudan maç nesnesine ekle (frontend için)
  if (homeStat) {
    match.homeForm    = homeStat.form;
    match.homeGoalAvg = homeStat.played > 0 ? (homeStat.gf / homeStat.played).toFixed(1) : "0.0";
  }
  if (awayStat) {
    match.awayForm    = awayStat.form;
    match.awayGoalAvg = awayStat.played > 0 ? (awayStat.gf / awayStat.played).toFixed(1) : "0.0";
  }

  // H2H çek
  const h2h = await fetchH2H(match.id);
  if (h2h) { match.headToHead = h2h; await sleep(6100); }
...
// Çorum FK haberi başlığına göre ilgili futbol fotoğrafı seç (Unsplash, ücretsiz)
// Her makale için FARKLI, konuyla ALAKALI görsel — aynı logo değil
const CORUM_IMG_POOL = {
  transfer: [
    "https://images.unsplash.com/photo-1556056504-5c7696c4c28d?w=900&q=85",  // imza/kalem
    "https://images.unsplash.com/photo-1551958219-acbc595aa24f?w=900&q=85",  // futbol topu
    "https://images.unsplash.com/photo-1543351611-58f69d7c1781?w=900&q=85",  // forma
  ],
  match: [
    "https://images.unsplash.com/photo-1508098682722-e99c43a406b2?w=900&q=85",  // stadyum gece
    "https://images.unsplash.com/photo-1540747913346-19212a4b3973?w=900&q=85",  // tribün
    "https://images.unsplash.com/photo-1431324155629-1a6deb1dec8d?w=900&q=85",  // saha
    "https://images.unsplash.com/photo-1574629810360-7efbbe195018?w=900&q=85",  // maç aksiyonu
  ],
  training: [
    "https://images.unsplash.com/photo-1517466787929-bc90951d0974?w=900&q=85",  // antrenman
    "https://images.unsplash.com/photo-1574629810360-7efbbe195018?w=900&q=85",  // koşu
  ],
  press: [
    "https://images.unsplash.com/photo-1560272564-c83b66b1ad12?w=900&q=85",  // basın toplantısı
  ...
  general: [
    ...
    "https://images.unsplash.com/photo-1540747913346-19212a4b3973?w=900&q=85",
    "https://images.unsplash.com/photo-1431324155629-1a6deb1dec8d?w=900&q=85",
    "https://images.unsplash.com/photo-1551958219-acbc595aa24f?w=900&q=85",
    "https://images.unsplash.com/photo-1517466787929-bc90951d0974?w=900&q=85",
    "https://images.unsplash.com/photo-1556056504-5c7696c4c28d?w=900&q=85",
    "https://images.unsplash.com/photo-1560272564-c83b66b1ad12?w=900&q=85",
  ],
};

let _corumImgCounter = 0; // index tracker to vary images

function pickCorumImage(title) {
  const t = (title || "").toLowerCase();
  let pool;
  if (["transfer","imza","talip","anlaş","sözleşme","kiralık","satın","bonservis","bedel","ücret"].some(k => t.includes(k)))
    pool = CORUM_IMG_POOL.transfer;
  else if (["maç","gol","skor","kazandı","yendi","berabere","mağlup","lig","puan","galibiyet","deplasman"].some(k => t.includes(k)))
    pool = CORUM_IMG_POOL.match;
  else if (["antrenman","hazırlık","kamp","çalışma","idman"].some(k => t.includes(k)))
    pool = CORUM_IMG_POOL.training;
  else if (["açıkla","konuştu","dedi","basın","toplantı","röportaj"].some(k => t.includes(k)))
    pool = CORUM_IMG_POOL.press;
  else
    pool = CORUM_IMG_POOL.general;

  const img = pool[_corumImgCounter % pool.length];
  _corumImgCounter++;
  return img;
}
...
async function enrichNews(item) {
  ...
  if (!image) {
    if (item.category === "corumsporhaber") {
      // Çorum FK: önce TheSportsDB/Wikipedia'dan kişi fotoğrafı dene
      const personImg = await resolveCorumImage(item.title || "");
      if (personImg) {
        image = personImg;
      } else if (item.link && !item.link.includes("news.google.com")) {
        // Google News dışı kaynaksa og:image dene
        const ogImg = await fetchOgImage(item.link);
        if (ogImg && !isGenericPlaceholder(ogImg)) image = ogImg;
      }
      // Hâlâ yoksa: konuya göre FARKLI futbol fotoğrafı seç (logo değil)
      if (!image) image = pickCorumImage(item.title || "");
      await sleep(80);
    } else {
      if (item.link) {
        const ogImg = await fetchOgImage(item.link);
